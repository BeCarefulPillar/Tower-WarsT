...
public class Packager {
    ...
    static void BuildFileIndex()
    {
        ...
        string[] paths = Directory.GetFiles(resDir, "*.*", SearchOption.AllDirectories);
        foreach (string path in paths)
        {
            if (path.EndsWith(".meta") || path.Contains(".DS_Store") || path.Contains(fileName))
                ignores.Add(path);
            else
                list.Add(string.Format("{0}|{1}", path.Replace(resDir, string.Empty).Replace('\\', '/'), Game.md5file(path)));
        }

        string str = string.Empty;
        for (int i = 0; i < ignores.Count; ++i)
            str += i == ignores.Count - 1 ? ignores[i] : ignores[i] + "\n";
        Debug.Log(str);

        File.WriteAllLines(resDir + fileName, list.ToArray());
    ...
    //            obj = objs[j];
    //            path = AssetDatabase.GetAssetPath(obj);
    //            if (obj && obj.name == ab.assetBundleName)
    //            {
    //                objs[j] = null;
    //                if(string.IsNullOrEmpty(path))
    //                {

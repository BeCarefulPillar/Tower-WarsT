...
public class UIPopupList : UIWidgetContainer
{
    ...
    [HideInInspector][SerializeField] protected bool isRetainButton = false;
    ...
    /// 是否保留主按钮为默认
    /// </summary>
    public bool IsRetainButton { get { return isRetainButton; } set { isRetainButton = value; } }
	...
	protected virtual void OnClick ()
	{
		...
		{
			if (isRetainButton) OnItemPress(mHighlightedLabel.gameObject, true);
            else CloseSelf();
        }
	}
	...
	public virtual void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
		{
			...
			if (openOn == OpenOn.Manual && mSelection != gameObject)
			{
                ...
                startingPosition.z = pTrans.position.z - mPanel.anchorCamera.transform.position.z;

...
public class Util
{
    ...
    public static void ClearChild(Transform go)
    {
        ...
        if(go && go.childCount>0)
        {
            Transform[] gos = new Transform[go.childCount];
            for (int i = 0; i < go.childCount; ++i)
                gos[i] = go.GetChild(i);
            for (int i = 0; i < go.childCount; ++i)
                UnityEngine.Object.Destroy(gos[i].gameObject);
        }
    }
    ...
    public static void ClearMemory()
    {
        ...
        LuaManager mgr = Game.Ins.LuaMgr;
    ...
    //    LuaManager luaMgr = AppFacade.Instance.GetManager<LuaManager>(ManagerName.Lua);
    //    if (luaMgr == null) return null;
    //    return luaMgr.GetFunction(funcName);
    //}
